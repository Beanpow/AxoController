...
class SafetyController:
    ...
    def record_callback(self, indx: int):
    ...
    def process_raw_data(self):
        ...
        max_indx = np.max(self.safe_moment_raw[:, 0]) + 1
        indx_set = [[] for i in range(max_indx)]
        for i in range(self.safe_moment_raw.shape[0]):
            indx_set[self.safe_moment_raw[i, 0]].append(i)
        ...
        self.safe_moment = np.array(safe_moment)
    ...
    def detect_callback(self, indx: int):
        moments = self.moment_menager.get_all_moments()[0]
        ...
        if np.any(moments < low_limitation) or np.any(moments > high_limitation):
            print(f"[error]: moment is not safe, indx: {indx}, moments: {moments}, low_limitation: {low_limitation}, high_limitation: {high_limitation}")
            self.axo_controller.exit_control_mode()
...
if __name__ == "__main__":
    ...
    try:
        ...
        # safety_controller.run(duration=1800)

        safety_controller.record_safe_info(4)
